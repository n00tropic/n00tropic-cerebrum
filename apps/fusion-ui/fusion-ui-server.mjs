...
const authUser = process.env.FUSION_AUTH_USER || "admin";
const authPass = process.env.FUSION_AUTH_PASS || "fusion";

function checkAuth(req, _res) {
	const auth = req.headers.authorization;
	if (!auth) return false;
	const [scheme, creds] = auth.split(" ");
	if (scheme !== "Basic" || !creds) return false;
...
const server = http.createServer((req, res) => {
	if (!checkAuth(req, res)) {
		res.writeHead(401, { "WWW-Authenticate": 'Basic realm="Fusion UI"' });
