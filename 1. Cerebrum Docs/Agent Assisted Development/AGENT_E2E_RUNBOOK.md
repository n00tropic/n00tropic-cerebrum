...
| **Cortex Catalog MCP** | `n00-cortex/mcp-server/`                | Exposes catalog JSON, boilerplates, and documentation manifests as read-only resources for planning phases.                                        |
| **Docs MCP**           | `mcp/docs_server/`                      | Supplies doc search/list/read utilities so agents can cite governing specs.                                                                        |

## 2. Installation Checklist

1. **Node dependencies** (once per workspace):
...
1. **Agent framework runtime (Python sample agents):**
...
2. **Planning/Architecture** – Use `n00-docs` + `n00-cortex` MCP servers to gather ADRs, manifests, and tag taxonomies.
...
## 5. Sample Agent Skeleton (Python)
...
- The helper degrades gracefully when `agent_framework` is missing, so existing scripts keep functioning without the dependency.

To view traces locally, launch an OpenTelemetry collector (or Docker `otel/opentelemetry-collector`) pointing at Honeycomb/Jaeger, then run `cli.py` commands—the spans are emitted before any subcommand logic executes.
...
1. **CLI Sanity**
   ...
   (Expect startup banners; terminate after confirming no missing-path errors.)
...
Following this runbook ensures agents can traverse every orchestration path (workspace health, project lifecycle, AI workflows, documentation lookups) using a consistent MCP interface.
