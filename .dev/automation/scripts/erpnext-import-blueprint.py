...
class ERPNextClient:
    ...
    def _request(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to decode ERPNext response from {url}: {raw!r}"
            ) from exc
...
def find_existing_project(
    client: ERPNextClient, display_name: str
    ...
    if isinstance(response, list) and response:
        ...
        full_details = unwrap(
            client.get(
                ...
                params={
                    "fields": json.dumps(
                        ["name", "project_name", "status", "project_detail"]
                    )
                },
            )
        )
...
def create_project(client: ERPNextClient, payload: Dict[str, Any]) -> Dict[str, Any]:
    response = unwrap(
        client.post("api/resource/Project", {"doctype": "Project", **payload})
    )
...
def find_existing_task(
    client: ERPNextClient, project_name: str, subject: str
) -> Optional[Dict[str, Any]]:
    filters = json.dumps(
        [
...
def fetch_task_details(client: ERPNextClient, name: str) -> Dict[str, Any]:
    ...
    response = unwrap(
        client.get(f"api/resource/Task/{quote_name(name)}", params={"fields": fields})
    )
...
def create_task(
    client: ERPNextClient, project_name: str, payload: Dict[str, Any]
) -> Dict[str, Any]:
    task_payload = {
        "doctype": "Task",
        "project": project_name,
...
def main() -> None:
    ...
    try:
        ...
        if existing_project:
            ...
            print(
                f"Project '{display_name}' already exists (name={project_name}); fields updated."
            )
        else:
        ...
        for task in task_payloads:
            ...
            if existing_task:
                ...
                if (details.get("description") or "").strip() != task[
                    "description"
                ].strip():
                ...
                if updates:
                    ...
                    print(
                        f"Task '{subject}' already up to date (name={existing_task['name']})."
                    )
            else:
