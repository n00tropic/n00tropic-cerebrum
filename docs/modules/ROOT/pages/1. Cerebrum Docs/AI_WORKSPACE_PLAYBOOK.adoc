...
| JSON goes to stdout *and* `artifacts/workspace-health.json`.

| Clean only-untracked noise (e.g. generated assets) safely
| `./.dev/automation/scripts/workspace-health.sh --clean-untracked`
| Runs `git clean -fd` for repos that have _no_ tracked changes.

| Realign submodule pointers
| `./.dev/automation/scripts/workspace-health.sh --sync-submodules`
| Equivalent to `git submodule sync && git submodule update --init --recursive`.

| Run everything (submodules + trunk sync)
| `./.dev/automation/scripts/workspace-health.sh --fix-all`
| Alias for the previous two steps.

| Strict CI-style enforcement
| Append `--strict-submodules --strict-root`
...
. *Root shows tracked changes to a submodule* (`workspace-health` prints `+tracked 1 … n00t+`):
...
. `workspace-health.sh --publish-artifact --json`.
. Read `artifacts/workspace-health.json` to determine the active repo + remediation path.
. Run guarded cleaners (`--clean-untracked`) or `--sync-submodules` as needed.
. When invoked via MCP, prefer `workspace.gitDoctor` so the host automatically records the output path.
