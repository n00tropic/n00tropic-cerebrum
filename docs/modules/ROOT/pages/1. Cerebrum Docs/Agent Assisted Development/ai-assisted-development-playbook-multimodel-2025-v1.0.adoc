= Artificial intelligence assisted development playbook (multi-model, 2025)
...
=== Google Gemini usage notes (2.5 pro / 2.5 flash / 2.5 flash-lite)
...
=== Anthropic Claude usage notes (Opus • Sonnet 3.5/4.x)
...
=== Meta Llama 3.x usage notes (open-weights family)
...
=== Mistral Codestral usage notes (latest 25.08 series)
...
* *Notes:* Great drop‑in for local/on‑prem coding assistance; keep prompts short
 to exploit FIM.
=== Cohere Command A usage notes (and Command A Vision)
...
=== DeepSeek Coder V2 usage notes (open)
