...
* `--publish-artifact` writes the JSON snapshot to
 `artifacts/workspace-health.json` so agents and ci can ingest it without
 rerunning the script.
* Add `--sync-submodules` when reviving a stale clone, or `--fix-all` to run
 submodule + trunk sync together.
* Use `--clean-untracked` to safely drop generated files; it only runs `git clean
...
TIP: wire the workflow above into automation pipelines (Trunk, ci) to fail fast
