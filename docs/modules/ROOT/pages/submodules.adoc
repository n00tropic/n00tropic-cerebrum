...
This page captures the crisp rules we use to keep the superrepo and every subrepo healthy, especially when agents are driving automation.
...
* Build artifacts (e.g., `.next/`, `dist/`, `storybook-static/`, temporary `artifacts/`).
* Local virtualenvs (`.venv/`, `venv/`) and `.ruff_cache/`, `.pytest_cache/`.
* Package managers’ stores (`node_modules/`, `.pnpm-store/`, `.cache/`).

== CI / environment notes

* Prefer `astral-sh/setup-uv@v2` + `uv sync` for Python in new workflows; it is faster and cached by default.
...
* A deeper guide in `n00menon` mapped into the doc site via Antora so agents and humans see the same source of truth.
