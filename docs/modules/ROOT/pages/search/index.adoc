...
Trigger the workflow manually (`Actions → search-reindex → Run workflow`) after updating secrets. The job spins up a local HTTP server, runs the scraper with the remote endpoint, and reports success/failure in the job summary.
