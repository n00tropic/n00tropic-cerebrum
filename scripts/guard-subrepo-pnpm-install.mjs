...
const allow = process.env.ALLOW_SUBREPO_PNPM_INSTALL === "1" || process.env.CI === "1";
const cwd = process.cwd();
const pkgPath = path.join(cwd, "package.json");
if (!fs.existsSync(pkgPath)) {
  console.error("[guard-subrepo-pnpm-install] package.json not found in CWD; run pnpm install from repo root.");
