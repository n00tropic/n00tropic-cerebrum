...
for (const entry of entries) {
  ...
  if (!fs.existsSync(path.join(submodulePath, ".git"))) {
    console.warn(
      chalk.yellow(
        `⚠️  ${relativePath} is not a git repository (or .git is missing).`,
      ),
    );
  ...
  if (!branch) {
    ...
    if (tag) {
      ...
      console.warn(
        chalk.yellow(`⚠️  ${relativePath} is in detached HEAD state (${sha}).`),
      );
    }
  } else {
    // On a branch
    ...
    if (behind && parseInt(behind) > 0) {
      console.warn(
        chalk.yellow(
          `⚠️  ${relativePath} is behind origin/${branch} by ${behind} commits.`,
        ),
      );
    }
    if (ahead && parseInt(ahead) > 0) {
      console.log(
        chalk.cyan(
          `ℹ️  ${relativePath} is ahead of origin/${branch} by ${ahead} commits.`,
        ),
      );
    }
  }
