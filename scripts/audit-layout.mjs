        ...
        try {
            ...
            if (!pkg.engines) {
                ...
                if (pkg.engines.node !== REQUIRED_ENGINES.node) {
                    repoErrors.push(`Incorrect node engine: "${pkg.engines.node}" (expected "${REQUIRED_ENGINES.node}")`);
                }
                if (pkg.engines.pnpm !== REQUIRED_ENGINES.pnpm) {
                    repoErrors.push(`Incorrect pnpm engine: "${pkg.engines.pnpm}" (expected "${REQUIRED_ENGINES.pnpm}")`);
                }
            }
