...
for (const f of files) {
  ...
  // Replace 'e.g.' and variants to 'for example'
  s = s.replace(/(\b|\()e\.g\./gi, "$1for example");
  s = s.replace(/for example,/gi, "for example,"); // ensure comma if it was there? No, e.g., -> for example,, -> fix
  s = s.replace(/for example,\s*,/g, "for example,");
  ...
  s = s.replace(/[\s\u2000-\u200A]*—[\s\u2000-\u200A]*/g, "—");
  // Digits followed by MB/GB/KB -> Digits Space Unit
  s = s.replace(/(\d+)(MB|GB|KB|kB)\b/g, "$1 $2");
